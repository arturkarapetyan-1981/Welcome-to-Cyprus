    ...
    <div className="max-w-7xl mx-auto px-4 py-10 select-none">
          ...
          >
            ...
            <div className="p-6">
              ...
              <p className="text-gray-600 mb-3 text-justify">
                ...
                {!expandedDescriptions[index] && item.description.split(' ').length > 20 && (
                  <button
                    onClick={() => toggleExpand(index)}
                    className="ml-1 text-[var(--mid-teal)] hover:underline text-sm cursor-pointer"
                  >
                   ... more
                  </button>
                )}
      ...
      {visibleCount < shuffledItems.length && (
        <div className="text-center mt-8">
          ...
          >
            {lang === 'en' ? 'Load More' : lang === 'gr' ? 'Φόρτωση περισσότερων' : 'Показать еще'}
          </button>
        </div>
      )}
    </div>
